...
test("rejects an unknown SGS variable without printing its value", () => {
  assert.throws(
    () =>
      assertLoadtestEnvironment({
        ...valid,
        JWT_AUDIENCEE: "synthetic-secret-value",
      }),
    (error) => {
      assert.match(String(error), /JWT_AUDIENCEE/);
      assert.doesNotMatch(String(error), /synthetic-secret-value/);
      return true;
    },
  );
});
...
for (const [name, mutation] of [
  ...
  [
    "Neon",
    {
      DATABASE_URL: `${["postgres", "ql"].join("")}://u:test@ep.example.neon.tech/db`,
    },
  ],
  ...
  test(`rejects ${name}`, () => {
    assert.throws(
      () => assertLoadtestEnvironment({ ...valid, ...mutation }),
      /REFUSED/,
    );
  });
}
