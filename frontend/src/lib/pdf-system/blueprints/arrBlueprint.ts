...
export async function drawArrBlueprint(
  ...
  const hasFinalPdfMetadata = Boolean(
    arr.document_code || arr.final_pdf_hash_sha256 || arr.pdf_generated_at || arr.emitted_by,
  );
  ...
  drawExecutiveSummaryStrip(ctx, {
    ...
    metrics: [
      ...
      },
      {
        ...
        tone: arr.nivel_risco === 'critico' || arr.nivel_risco === 'alto' ? 'warning' : 'default',
      },
      {
        label: 'Probabilidade',
  ...
  drawMetadataGrid(ctx, {
    ...
    fields: [
      ...
      // The database creation timestamp is an audit fact and must not be
      // presented as the historical date of the field activity.
      { label: 'Data do documento', value: formatDate(arr.data) },
